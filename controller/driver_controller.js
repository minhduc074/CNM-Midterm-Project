    ...
    driver_db.get_all().then(driver_list => {
      ...
      for (var i = 0; i < rejected.length; i++) {
        for (var j = 0; j < driver_list.length; j++) {
          if (driver_list[j].status != 1 || driver_list[j].geocoding == "") {
            driver_list.splice(i, 1);
          } else if (rejected[i].username == driver_list[j].username) {
  ...
  driver.find_driver(lat_lng, rejected).then(result => {
    ...
    driver_db.get_all().then(driver_list => {
      for (var i = 0; i < driver_list.length; i++) {
        if (driver_list[i].status != 1 || driver_list[i].geocoding == "") {
          driver_list.splice(i, 1);
        } else {
...
driver.post("/address/", verifyAccessToken, (req, res) => {
  ...
  driver_db
    .update_address(request.username, request.address, geocoding)
    .then(resolve => {
      res.writeHead(200, {
        "Content-Type": "text/json"
      });
...
driver.post("/login/", (req, res) => {
  ...
  driver_db
    ...
    .then(driver => {
      ...
      ticket
        .updateRefreshToken("driver_" + username, refreshToken)
        .then(() => {
          res.writeHead(200, {
            "Content-Type": "text/json"
          });
